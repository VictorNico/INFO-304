        ...
        for word in data_arr:
            if '->' in word:
                ...
                if(process[0] == 'states'):
                    ...
                    operation.error_msg(
                        "illegal DFA tuple declaration: %s" % process[0])
            else:
                pass
            ...
            for transition in data:
                ...
                for transition_data in transition:
                    ...
                    for transition_data_value_name in transition_data:
                        ...
                        transition_data_value_name_map = map(
                            str, transition_data_value_name)
                        array_token_list = ""
...
class operations:
    ...
    def print_DFA_diagram(self, transition):
        ...
        for i in transition:
            ...
            value = i[1]
            to_append = to_append + \
                '%s -> %s [label="%s"];' % (initial, target, value)
    ...
    def tokenize(self, arr):
        ...
        for word in tokenize:
            if re.match('^[a-z,A-Z]', word):
                ...
                self.error_msg(
                    "Error in parsing: illegal character: %s " % word)
        print(tokenize_all)
                    ...
                    while i < len(arr):
                        if(self.greedy_scanner(arr, i) == 1 and re.match('^[a-z,A-Z]', arr[i])):
                            ...
                            if(arr[i] == ')' and arr[i+1] == '*'):
                                to_append = to_append + \
                                    '%s -> %s;' % (initial, start_in)
                                to_append = to_append + \
                                    '%s -> %s;' % (start_in, initial)
                                i += 2
                            else:
                                to_append = to_append + \
                                    '%s -> %s [label="%s"];' % (
                                        initial, target, arr[i])
                                initial = initial + 1
                                target = target+1
                                i += 1
    ...
    def main(self):
        ...
        while choice != '3':
            ...
            if(choice == '2'):
                ...
                dfa = DFA(states, alphabet, transition,
                ...
                while input_user_choice != '3':
                    ...
                    input_user_choice = raw_input(
                        "1 to generate random language\n2 for manual input \n3 go back\n\n: ")
                    if(input_user_choice == '1'):
                        # get length of language to be generated from user
                        input_user = raw_input(
                            "Enter Lenght of language to be generated: ")
                        ...
                        generated_value = self.generate_language(
                            number_to_generate, string_alphabet)
                        ...
                        stop = raw_input(
                            "Press any key to continue...(except poweroff!)")
